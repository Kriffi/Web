<?php
    ...
    function fnGetCardProfile($login){
        ...
        $sql = sprintf("SELECT `number`, `kol_kom`, `message`, `status` FROM `application` INNER JOIN `user` ON `application`.`user_id` =  `user`.`user_id` WHERE `application`.`user_id` = '%s' ORDER BY `application_id` DESC", $id);
        ...
        while($row = $result->fetch_assoc()){
            if($row['status'] == 'Отменен'){
                $data .= sprintf('<div class="card w-100 mb-3 mt-3 text-muted">
                                <div class="card-body">
                                    <h5 class="card-title">Номер заявки №%s</h5>
                                    <p class="mb-1"><span class="fw-semibold">Статус:</span> %s</p>
                                    <p class="mb-1"><span class="fw-semibold">Количество комнат:</span> %s</p>
                                    <p class="card-text">%s</p>
                                </div>
                            </div>', $row['number'], $row['status'], $row['kol_kom'], $row['message']);
    ...
    function fnGetCardAdmin(){
        ...
        $sql = "SELECT `application_id`,`surname`, `name`, `patronymic`, `number`, `kol_kom`, `message`, `status` FROM `application` INNER JOIN `user` ON `application`.`user_id` =  `user`.`user_id` ORDER BY `application_id` DESC";
        ...
        while($row = $result->fetch_assoc()){
            if($row['status'] == 'Отменен'){
                $data .= sprintf('<div class="card w-100 mb-3 mt-3 text-muted">
                            ...
                            </div>', $row['number'], $row['surname'], $row['name'], $row['patronymic'], $row['status'], $row['kol_kom'], $row['message']);
            }elseif($row['status'] == 'Подтвержден'){
                $data .= sprintf('<div class="card w-100 mb-3 mt-3 text-success">
                                <div class="card-body">
                                    <h5 class="card-title">Номер заявки №%s</h5>
                ...
                $data .= sprintf('<div class="card w-100 mb-3 mt-3">
                                ...
                                </div>', $row['number'], $row['surname'], $row['name'], $row['patronymic'], $row['status'], $row['kol_kom'], $row['message'], $row['application_id'], $row['application_id']);
            }
